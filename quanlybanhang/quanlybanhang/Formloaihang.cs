...
namespace quanlybanhang
{
    ...
    {
        SqlConnection Kn = new SqlConnection(@"Data Source=(local)\SQLEXPRESS;Initial Catalog=QLBH;Integrated Security=True");
        ...
        private void btnxoa_Click(object sender, EventArgs e)
        {
            ...
            {
                if (kiemtramatheoten() == true)
                {
                    MessageBox.Show("có mặt hàng thuộc loại này không thể xóa", "chú ý",
                        MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }

                else
                {
                    SqlCommand cmd = new SqlCommand(@"DELETE [dbo].[LOAIHANG] WHERE MaLoaiHang = '" + txtmh.Text + "' AND TenLoaiHang = N'" + txtth.Text + "'");
                    if (Kn.State == ConnectionState.Closed) Kn.Open();
                    cmd.Connection = Kn;
                    int Kq = cmd.ExecuteNonQuery();
                    if (Kq != 0)
        ...
        private bool kiemtramatheoten()
        {
            SqlCommand cmd = new SqlCommand(@"SELECT count(*)
            FROM [dbo].[MATHANG] WHERE MALOAIHANG = N'" + txtth.Text + "'");
            cmd.Connection = Kn;
            if (Kn.State == ConnectionState.Closed) Kn.Open();
            int Kq = (int)cmd.ExecuteScalar();
            if (Kq != 0) return true;
